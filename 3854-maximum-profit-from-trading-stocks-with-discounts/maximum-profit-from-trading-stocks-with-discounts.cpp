...
public:
    ...
    void dfs(int u, const vector<vector<int>>& tree, const vector<int>& present, const vector<int>& future, vector<vector<vector<int>>>& dp) {
        vector<int> skipBranch(B + 1, -1e9), buyBranch(B + 1, -1e9);
        skipBranch[0] = 0;
        buyBranch[0] = 0;
        for (auto v : tree[u]) {
