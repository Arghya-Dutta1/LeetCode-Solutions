...
public:
    ...
    int dp(int m, int k, int i, unsigned carry, const vector<int>& nums,
        ...
        for (int count = 0; count <= m; ++count) {
            ...
            res = (res + static_cast<long>(dp(m - count, k - (newCarry % 2), i + 1, newCarry / 2, nums, mem, comb)) * contribution) % kMod;
        }
    ...
    int magicalSum(int m, int k, vector<int>& nums) {
        ...
        vector<vector<vector<vector<int>>>> mem(m + 1, vector<vector<vector<int>>>(k + 1, vector<vector<int>>(nums.size() + 1, vector<int>(m + 1, -1))));
